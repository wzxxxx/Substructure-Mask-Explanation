...
for index, row in summary_data.iterrows():
    ...
    if row.attribution_mean<=0:
        g.text(atribution_x_cor, index+0.2, round(row.attribution_mean, 3), color="green",  ha="center")
        # g.text(index, -1.3, fg_rmse[index], color="blue",  ha="center") # 加准确率
    else:
        g.text(atribution_x_cor, index+0.2, round(row.attribution_mean, 3), color="red",  ha="center")
        # g.text(index, -1.3,  fg_rmse[index], color="blue",  ha="center") # 加准确率
