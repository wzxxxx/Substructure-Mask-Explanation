...
#     for i in range(10):
#         SMEG_explain_for_sar_optimization(seed=i, task_name='{}_6_brics_mol'.format(task), model_name=task,
...
#     for i in range(10):
#         seed = i + 1
#         result = pd.read_csv('../prediction/mol/{}_6_brics_mol_mol_{}_test_prediction.csv'.format(task, seed))
#         # 合并五个随机种子结果，并统计方差和均值
#         if seed == 1:
...
#         os.makedirs(dirs)
#     result_summary.to_csv('../prediction/summary/{}_6_brics_mol_mol_prediction_summary.csv'.format(task), index=False)

# for task in ['ESOL', 'hERG', 'Mutagenicity']:
#     with open('../result/hyperparameter_{}.pkl'.format(task), 'rb') as f:
#         hyperparameter = pkl.load(f)
#     for i in range(10):
#         SMEG_explain_for_sar_optimization(seed=i, task_name='{}_top20_6_brics_mol'.format(task), model_name=task,
#                                           rgcn_hidden_feats=hyperparameter['rgcn_hidden_feats'],
#                                           ffn_hidden_feats=hyperparameter['ffn_hidden_feats'],
#                                           lr=hyperparameter['lr'], classification=hyperparameter['classification'])
#     # 将训练集，验证集，测试集数据合并
...
#     for i in range(10):
#         seed = i + 1
#         result = pd.read_csv('../prediction/mol/{}_top20_6_brics_mol_mol_{}_test_prediction.csv'.format(task, seed))
#         # 合并五个随机种子结果，并统计方差和均值
#         if seed == 1:
#             result_summary['smiles'] = result['smiles']
#             result_summary['label'] = result['label']
...
#     if not os.path.exists(dirs):
#         os.makedirs(dirs)
#     result_summary.to_csv('../prediction/summary/{}_top20_6_brics_mol_mol_prediction_summary.csv'.format(task), index=False)



# multiple_property
for task in ['hERG', 'Mutagenicity']:
    with open('../result/hyperparameter_{}.pkl'.format(task), 'rb') as f:
        hyperparameter = pkl.load(f)
    for i in range(10):
        SMEG_explain_for_sar_optimization(seed=i, task_name='{}_top20_6_brics_mol'.format('Non-hERG-Non-Mutag'), model_name=task,
                                          rgcn_hidden_feats=hyperparameter['rgcn_hidden_feats'],
                                          ffn_hidden_feats=hyperparameter['ffn_hidden_feats'],
                                          lr=hyperparameter['lr'], classification=hyperparameter['classification'])
    # 将训练集，验证集，测试集数据合并
    ...
    for i in range(10):
        seed = i + 1
        result = pd.read_csv('../prediction/mol/{}_top20_6_brics_mol_mol_{}_test_prediction_cache.csv'.format('Non-hERG-Non-Mutag', seed))
        # 合并五个随机种子结果，并统计方差和均值
        if seed == 1:
            result_summary['smiles'] = result['smiles']
            result_summary['label'] = result['label']
    ...
    if not os.path.exists(dirs):
        os.makedirs(dirs)
    result_summary.to_csv('../prediction/summary/{}_model_top20_6_brics_mol_mol_prediction_summary.csv'.format('Non-hERG-Non-Mutag-'+task), index=False)
