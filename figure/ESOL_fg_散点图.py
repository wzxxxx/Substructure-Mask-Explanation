...
# print(sub_name_list)
# g = sns.barplot(x="sub_name", y="attribution", ci='sd', capsize=0.03, errwidth=1, data=data, n_boot=2000, order=sub_name_list)
# g = sns.barplot(x="sub_name", y="attribution", ci=None, palette='RdYlGn_r', capsize=0.03, errwidth=1, data=data, n_boot=2000, order=sub_name_list)
# g = sns.barplot(x="sub_name", y="attribution", ci=None, palette='RdYlGn', capsize=0.03, errwidth=1, data=data, n_boot=2000, order=filter_sub_name_list)
# g = sns.violinplot(x="sub_name", y="attribution", data=data, palette='RdYlGn', order=filter_sub_name_list)
g = sns.stripplot(y="sub_name", x="attribution", hue='attribution_tag', palette=colors, data=data,  jitter=0.15, order=filter_sub_name_list, size=4)

atribution_x_cor=-3.1
for index, row in summary_data.iterrows():
    print(index)
    if row.attribution_mean<=0:
        g.text(atribution_x_cor, index+0.1, round(row.attribution_mean, 3), color="red",  ha="center")
        # g.text(index, -1.3, fg_rmse[index], color="blue",  ha="center") # 加准确率
    else:
        g.text(atribution_x_cor, index+0.1, round(row.attribution_mean, 3), color="green",  ha="center")
        # g.text(index, -1.3,  fg_rmse[index], color="blue",  ha="center") # 加准确率
