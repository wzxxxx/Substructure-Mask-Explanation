...
for task_name in task_name_list:
    data = pd.read_csv('../prediction/attribution/{}_brics_attribution_summary.csv'.format(task_name))
    ...
    brics_frags_data.to_csv('../brics_build_mol/{}_brics_frag.csv'.format(task_name), index=False)
    ...
    for frag_num in [6]:
        negative_brics_smi_list = brics_mol_generator(frag_num=frag_num, same_frag_combination_mol_num=10, mol_number=3000, seed=2022, frags_list=negative_brics_frags)
        positive_brics_smi_list = brics_mol_generator(frag_num=frag_num, same_frag_combination_mol_num=10, mol_number=3000, seed=2022, frags_list=positive_brics_frags)
        ...
        brics_mol_data['label'] = labels
        brics_mol_data.to_csv('../brics_build_mol/{}_{}_brics_mol.csv'.format(task_name, frag_num), index=False)# 将生成的分子按照进行保存
